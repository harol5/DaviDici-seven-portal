...
<section class="content">
    <section>
        <section>
            <h1>Project Punch List</h1>
            <h2>Aqua Reserve - New Post Richey, Florida</h2>
        </section>
        <section>
            <p class="">By check marking each product installed as completed, you are confirming that:</p>
            <ul class="">
    ...
    <section>
        @foreach ($data as $label => $content)
            @if( $label === 'unitOne' || $label === 'unitTwo' || $label === 'unitThree' || $label === 'unitFour' || $label === 'unitClub')
                <section>
                    <h1>{{  }}</h1>
                    @foreach ($data['unitOne'] as $product => $state)
                        <article></article>
                    @endforeach
                </section>
            @endif
